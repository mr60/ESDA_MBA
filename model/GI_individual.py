...
def LIDGIsubcat_area(use, typeGI, filename, subcatchments):
    '''
    ...
    for index, row in df.iterrows():
        if row['Name'] in subcatchments:
            ind=areas[areas['Name']==row['Name']].index
            if typeGI=='BioCell':
                new_imperv=(areas.loc[ind,'imperv_area']/(areas.loc[ind,'total_area']-areas.loc[ind,'perv_area']*(use/100)))*100
                df.loc[index,'%Imperv']=new_imperv.values[0]
                ...
                new_imperv=((areas.loc[ind,'imperv_area']-areas.loc[ind,'build_area']*(use/100))/(areas.loc[ind,'total_area']-areas.loc[ind,'build_area']*(use/100)))*100
                df.loc[index,'%Imperv']=new_imperv.values[0]
                              
            elif typeGI=='PerPav':
                new_imperv=((areas.loc[ind,'imperv_area']-(areas.loc[ind,'imperv_area']-areas.loc[ind,'build_area'])*(use/100))/(areas.loc[ind,'total_area']-(areas.loc[ind,'imperv_area']-areas.loc[ind,'build_area'])*(use/100)))*100
                df.loc[index,'%Imperv']=new_imperv.values[0]
    ...
    fin='input_files/'+filename+str(hash(''.join(subcatchments)))+f'use_{use}'+f'{typeGI}'+'.inp'
...
def simulGI_nodes(i,filename):
    ...
    LIDGIsubcat_area(use,typeGI,filename,i)

    fin=filename+str(hash(''.join(i)))+f'use_{use}'+f'{typeGI}'
...
def save_results(j,storm,output):
    ...
    for i in range(0, len(output)):
        for m in range(0,len(results)):
            output[i][m].to_csv(f'results/one/simulation_{storm}_use100_{j}_{i}_{results[m]}')
