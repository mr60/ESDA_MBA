...
def LIDsubcat_area(filename,subcatchments,area):
    '''
    ...
    fin=filename+str(hash(''.join(subcatchments)))+f'area_{area}'+'.inp'
...
def set_rs(filename):
    ...
    with Simulation(filename+'.inp') as sim:
        for sub in tqdm(Subcatchments(sim)):
            subcat.append(sub.subcatchmentid)
            
    from random import sample
    
    rs=[]
    for i in range(1,len(subcat)):
        rs.append(sample(subcat,i))
    return rs
...
def simul_nodes(i,filename):
    ...
    LIDsubcat_area(filename,i,area)
...
def save_results(j,rs,storm,output):
    ...
    for i in range(0, len(output)):
        output[i].to_csv(f'results/simulation_{storm}_{j}_{i}')
    ...
    for i in rs:
        rswrite=rswrite+'\n'+','.join(i)
    f=open(f'results/rs{storm}_{j}','w')
    f.write(rswrite)
    f.close()
